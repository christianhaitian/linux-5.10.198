export CROSS_COMPILE=aarch64-linux-gnu- && make ARCH=arm64 x55_defconfig && make ARCH=arm64 KERNEL_CONFIG=x55_defconfig -j$(nproc)
export CROSS_COMPILE=aarch64-linux-gnu- && make ARCH=arm64 x55_defconfig && make ARCH=arm64 KERNEL_CONFIG=x55_defconfig modules -j$(nproc)
